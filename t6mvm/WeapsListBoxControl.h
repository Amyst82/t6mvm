...
namespace Weapon
{
	...
	public:
		...
		UI_WeapsListBox(const char* title, float x, float y, float width, float height, bool drawRelative, int* selectedIndex, T6SDK::AnchorPoint anchorPoint, uintptr_t function)
		{
			InternalSelected = 0;
			Title = title;
			X = x;
		...
		/// <param name="anchorPoint"></param>
		/// <param name="function">Pointer to a function that will be called when the selected item is changed. void func(int)</param>
		UI_WeapsListBox(const char* title, int gridColumn, int gridRow, float width, float height, int* selectedIndex, T6SDK::AnchorPoint anchorPoint, uintptr_t function)
		{
			Title = title;
		...
		UI_WeapsListBox(const char* title, int gridColumn, int gridRow, int gridColumn2, int gridRow2, int* selectedIndex, T6SDK::AnchorPoint anchorPoint, uintptr_t function)
		{
			Title = title;
			GridColumn = gridColumn;
			GridRow = gridRow;
		...
		void Draw()
		{
			if (*SelectedIndex == -1 || WeaponAnimations::Weapons.size() == 0)
				return;
			if (!IsEnabled)
			{
				Clicked = false;
			...
			int endIndex = min(startIndex + visibleItemCount, static_cast<int>(WeaponAnimations::Weapons.size()));
			...
			for (int i = startIndex; i < endIndex; i++)
			{
				...
				for (auto& anim : WeaponAnimations::Weapons[i].AnimationsMap)
				{
				...
				T6SDK::Drawing::DrawTextAbsolute(WeaponAnimations::Weapons[i].FriendlyName, bgRect.left + 5, itemY + 0, 1.0f, !IsEnabled ? T6SDK::Drawing::GRAYCOLOR : Hovered ? T6SDK::Drawing::ORANGECOLOR : hasReplacements ? T6SDK::Drawing::GREENCOLOR : T6SDK::Drawing::WHITECOLOR, T6SDK::AnchorPoint::TopLeft, &textRect);
			}
			// Draw the scrollbar
		...
		void handleMouseClick(float mouseX, float mouseY, RECT* rect)
		{
			...
			else
			{
				if (mouseX >= (float)rect->left && mouseX <= (float)(rect->left + m_width - m_scrollbarWidth))
				{
					...
					if (clickedIndex >= 0 && clickedIndex < WeaponAnimations::Weapons.size())
					{
