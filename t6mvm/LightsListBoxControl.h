...
namespace Lights
{
	...
	public:
		...
		UI_ListBox(const char* title, float x, float y, float width, float height, bool drawRelative, vector<LightDef>* items, int* selectedIndex, T6SDK::AnchorPoint anchorPoint, uintptr_t function)
		{
			Title = title;
			X = x;
			Y = y;
		...
		/// <param name="anchorPoint"></param>
		/// <param name="function">Pointer to a function that will be called when the selected item is changed. void func(int)</param>
		UI_ListBox(const char* title, int gridColumn, int gridRow, float width, float height, vector<LightDef>* items, int* selectedIndex, T6SDK::AnchorPoint anchorPoint, uintptr_t function)
		{
			Title = title;
		...
		UI_ListBox(const char* title, int gridColumn, int gridRow, int gridColumn2, int gridRow2, vector<LightDef>* items, int* selectedIndex, T6SDK::AnchorPoint anchorPoint, uintptr_t function)
		{
			Title = title;
			GridColumn = gridColumn;
			GridRow = gridRow;
		...
		void Draw()
		{
			...
			int endIndex = min(startIndex + visibleItemCount, static_cast<int>(Items->size()));
			...
			for (int i = startIndex; i < endIndex; ++i)
			{
				...
				char buffer[100];
				sprintf(buffer, "%s LIGHT #%i", (*Items)[i].spotLight ? "SPOT" : "OMNI", i + 1);
				T6SDK::Drawing::DrawTextAbsolute(buffer, bgRect.left + 5, itemY + 0, 1.0f, !IsEnabled ? T6SDK::Drawing::GRAYCOLOR : Hovered ? T6SDK::Drawing::ORANGECOLOR : T6SDK::Drawing::WHITECOLOR, T6SDK::AnchorPoint::TopLeft, &textRect);
			}
			// Draw the scrollbar
		...
		void drawScrollbar(RECT* rect)
		{
			...
			float scale = 1.0f / 1080.0f * static_cast<float>(T6SDK::Addresses::ScreenHeight.Value());
			float itemHeight = (float)T6SDK::Drawing::normalFont->pixelHeight * scale;
			int totalContentHeight = Items->size() * itemHeight; // Total height of all items
